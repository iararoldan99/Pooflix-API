...
public class PeliculaController {
  ...
  /*
   * @GetMapping("/api/pooflix/peliculas/{_directorId}") public
   * ResponseEntity<Pelicula>
   * buscarPeliculaPorDirector(@RequestParam(value="director") ObjectId
   * _directorId) { Pelicula pelicula =
   * peliculaService.obtenerPeliculaPorDirector(_directorId); if (pelicula ==
   * null) return ResponseEntity.notFound().build(); return
   * ResponseEntity.ok(pelicula);
   * 
   * }
   * 
   * @GetMapping("/api/pooflix/peliculas/{_actorId}") public
   * ResponseEntity<Pelicula> buscarPeliculaPorActor(@PathVariable ObjectId
   * _actorId) { Pelicula pelicula =
   * peliculaService.obtenerPeliculaPorActor(_actorId); if (pelicula == null)
   * return ResponseEntity.notFound().build(); return ResponseEntity.ok(pelicula);
   * 
   * }
   * 
   * @PostMapping("/peliculas/{_id}/actores") public
   * ResponseEntity<GenericResponse> asignarActor(@PathVariable ObjectId _id,
   * 
   * @RequestBody PeliculaAsigActor paa) { GenericResponse gR = new
   * GenericResponse(); if (peliculaService.asignarActor(_id, paa._idActor)) {
   * 
   * gR.isOk = true; gR.mensaje = "Actor asignado con éxito a la pelicula"; return
   * ResponseEntity.ok(gR); } gR.isOk = false; gR.mensaje =
   * "El actor no pudo ser asignado."; return
